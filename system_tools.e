...
feature -- FIXED: removed {C_PRETTY_PRINTER}
...
feature -- FIXED: removed {ID_PROVIDER}
...
feature {NONE}
	...
	add_external_lib(lib: STRING) is
	 ...
	 not lib.is_empty
		do
	 token_buffer.copy(lib);
	 token_buffer.extend_unless(' ');
	 if not external_lib.has_substring(token_buffer) then
		 append_token(external_lib,token_buffer);
	 end;
		end;
